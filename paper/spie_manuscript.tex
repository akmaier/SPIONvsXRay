...
The particles are iron oxide, not pure iron. We model the magnetite core
($\mathrm{Fe_3O_4}$), whose iron mass fraction is $72.4\%$; the bound oxygen is
nearly tissue-equivalent and adds only a few percent to the attenuation per gram of
iron. Rather than assume a fixed vial concentration, we spread a fixed particle
...
mixture rule, so a zero-iron insert equals background exactly.
...
projections over $198^\circ$ -- and inserted a SPION tumor into the rabbit volume
(Fig.~\ref{fig:rabbit}); a cone-beam forward projection and FDK reconstruction of
that phantom is the immediate next step.

\begin{figure}[ht]
\centering
\includegraphics[width=0.92\linewidth]{rabbit_slices.png}
\caption{Real rabbit anatomy for the planned three-dimensional study: orthogonal
slices of the post-mortem RabbitCT reference volume, whose C-arm geometry
($745/1196\,\mathrm{mm}$, $496$ views over $198^\circ$) we extracted from the
benchmark dataset.}
\label{fig:rabbit}
\end{figure}
